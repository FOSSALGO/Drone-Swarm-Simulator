...
public class Drone {
    protected int indexI, indexJ, sensoreFoV, cameraFoV, cellSize;
    ...
    protected void setIndex(int indexI, int indexJ) {
        this.indexI = indexI;
        this.indexJ = indexJ;
    }
    ...
    protected void draw(GraphicsContext gc, int indexI, int indexJ, int[][] environment) {
        setIndex(indexI, indexJ);
        draw(gc, environment);
    }

    protected void draw(GraphicsContext gc, int[][] environment) {
        ...
        if (droneX >= 0 && droneY >= 0) {
            ...
            gc.setFill(Color.valueOf("#FF7D29"));//("#FFCF50"));//("#00F7FF"));//("#F13E93"));//("#393E46"));
    ...
    protected void drawCameraVisibility(GraphicsContext gc, double sx, double sy, int[][] environment) {
    ...
    protected void drawSensoreVisibility(GraphicsContext gc, double sx, double sy, int[][] environment) {
        int numRows = environment.length;
        int numCols = environment[0].length;
        int rays = 360;
        double[] xPoints = new double[rays];

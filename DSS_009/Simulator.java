...
public class Simulator extends Application {
    ...
    private final int numberOfDrones = 5;// DRONES
    ...
    private void setFree(int ci, int cj) {
        // setiap drone memilikifrontiernya masing-masing, namun sebuah drone dapat menghapus frontier dronelain saat setfree
        int rays = 360;
        double cSize = cellSize * scale;
        double hcSize = 0.5 * cSize;
        ...
        for (int i = 0; i < rays; i += 2) {
            ...
            while (checked < MAX_RADIUS) {
                ...
                if (row < 0 || row >= map.length || col < 0 || col >= map[0].length || map[row][col] == -1) {
                    ...
                    for (int d = 0; d < activeDrones.size(); d++) {
                        if (!frontiers[d].isEmpty()&&frontiers[d].contains(freePoint)) {
    ...
    private void initializeExploration(){
        if(activeDrones!=null&&map!=null&&!activeDrones.isEmpty()){
            for (int d = 0; d < activeDrones.size(); d++) {
                int ci = activeDrones.get(d).indexI - MIN_ROW;
                int cj = activeDrones.get(d).indexJ - MIN_COL;
                setFree(ci, cj);
    ...
    private void update() {
        ...
        //check keyboard
        if(inputKeyboard.contains("R")&&!explore){
            explore= true;

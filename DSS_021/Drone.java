...
public class Drone {
    public CameraFoVType cameraFOVType;
    public int indexI, indexJ, sensoreFoV, cameraFoV, cellSize;
    public String name = "";

    public Drone(int indexI, int indexJ, int sensoreFoV, int cameraFoV, int cellSize, CameraFoVType cameraFOVType) {
        this.indexI = indexI;
        this.indexJ = indexJ;
        this.sensoreFoV = sensoreFoV;
        this.cameraFoV = cameraFoV;
        this.cellSize = cellSize;
        this.cameraFOVType = cameraFOVType;
    }
    ...
    private void drawCameraVisibility(GraphicsContext gc, double sx, double sy, int[][] environment) {
        if (cameraFOVType.equals(CameraFoVType.CIRCLE)) {
            ...
            double cSize = cellSize;//tidak perlu dikalikan dg scale karena dia dipanggil di dalam method render setelah mengatur ulang scale
            double hcSize = 0.5 * cSize;
            double radius = (double) cameraFoV * cSize + hcSize;
            double x0 = x - radius;
            double y0 = y - radius;
            double x1 = x + radius;
            double y1 = y + radius;
            gc.fillRect(x0, y0, 2 * radius, 2 * radius);
        }
    }
